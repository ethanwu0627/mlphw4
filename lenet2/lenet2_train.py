...
from lenet2 import build_lenet2
...
def main():
    ...
    # Data preprocessing: pad 28x28 to 32x32 + data augmentation for training
    # Training transform: augmentation + padding
    # Note: Small rotations and translations only - large rotations would change digit meaning
    train_transform = transforms.Compose([
        transforms.RandomAffine(
            degrees=15,  # Small rotation up to ±15°
    ...
    # Test transform: only padding (no augmentation)
    test_transform = transforms.Pad(2, fill=0, padding_mode='constant')
    
    # Load datasets
    print('Loading datasets...')
    train_dataset = MNISTParquet(split='train', transform=train_transform)
    test_dataset = MNISTParquet(split='test', transform=test_transform)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    
    # Build model
    print('Building modified LeNet5 model (LeNet2)...')
    print('Modifications:')
    print('- ReLU activation instead of scaled tanh')
    print('- Dropout layers (rate=0.3) for regularization')
    print('- Data augmentation: random rotations (±15°), translations, and scaling')
    model = build_lenet2(dropout_rate=0.3)
    ...
    plt.title('LeNet5 Modified (LeNet2) Training and Test Error Rates', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('lenet2_error_rates.png', dpi=150)
    print('Error rate plot saved to lenet2_error_rates.png')
    plt.show()
